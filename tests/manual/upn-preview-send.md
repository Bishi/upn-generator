...
- [ ] Confirm Download All PDFs records a saved PDF delivery event for each successfully written apartment packet.
- [ ] Reload the UPN page and confirm saved PDF status is restored for apartment rows.
- [ ] Confirm a mixed month with some emailed packets and some saved PDF packets marks the UPN workflow step complete.
...
- [ ] Change packet content after a prior send/save and confirm the prior delivery is shown as no longer current until sent or saved again.
