...
Current status: **v0.4.10. Phases 2 + 3 largely complete, including provider-based split rules, equal apartment split support, chimney-service provider support, image-based bill OCR import, import timeout protection, corrected JP VOKA split defaults, OCR-tolerant Dimnikar parsing, stronger OCR normalization, review-state warnings for fallback parses, lighter hover-only review indicators, improved year/month navigation, multi-bill PDF import stability fixes, and multi-recipient apartment emails. Phase 4 (email + keyring) next.**
...
```bash
git tag v0.4.10 && git push origin main && git push origin v0.4.10
```
