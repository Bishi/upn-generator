...
| **Download All PDFs** | Saves one combined UPN packet PDF per apartment to a folder of your choice |
...
UPN Preview keeps delivery history for the selected month. After reload, apartment rows can show sent email, saved PDF, failed, blocked, partial, or changed status based on current packet hashes and persisted delivery events.
...
Manual backups are saved wherever you choose as `.sqlite3` files. They contain app data, the selected appearance theme, inbox import history, and UPN email/PDF delivery history, but intentionally exclude saved SMTP and inbox passwords. Mail passwords are stored in Windows Credential Manager and are matched to the configured username before use.
